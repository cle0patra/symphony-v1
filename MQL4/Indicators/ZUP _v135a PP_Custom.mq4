...
// Паттерны Gartley
 ...
 color  ExtColorPatterns        = DarkOrange; // Но добавленные паттерны -  без режима 7 
...
//
 ...
 int    NumberPattern           = 1;  // Номер паттерна, по которому калибруется зигзаг и параметры которого выводятся через InfoTF
...
//--------------- Custom 5-Point Pattern
 int    CustomPattern           = 1; // 0 - не выводится пользовательский паттерн 
                                           // 1 - выводится вместе с основной группой паттернов
                                           // 2 - выводится только custom паттерны
 double minXB                   = 1.128; // 2;  //0.5;  Это   HENRY  поручик 
...
//----
 ...
 int    ExtPoint=11; // количество точек зигзага для зигзага Talex 
// Параметры для зигзага, разработанного wellx
 ...
 int    ExtMaxBar               = 1400;     // Количество баров обсчёта (0-все)
 int    ExtMinBar               = 0;
...
//-------------------------------------
 ...
 color  ExtPesavento            = Black; // Yellow; Цвета для белого фона. Убрал цвета для черного фона 
...
// Сигнальные линии 50% медианы
 ...
 bool   ExtFSLShiffLinesDinamic      = true; //false; // вывод линии FSL линий Шиффа для динамических вил Эндрюса
 color  ExtFSLShiffLinesDinamicColor = DarkOrange; //MediumSlateBlue; // MediumBlue; // DarkKhaki; //
 bool   ExtFSLShiffLinesStatic       = true; // false; // вывод линии FSL линий Шиффа для статических вил Эндрюса
...
// Переменные для фибовееров
 bool   ExtFiboFanDinamic     = false;  // может выводиться самостоятельно
 bool   ExtFiboFanStatic      = false;  // выводится только совместно со статическими вилами
...
// Переменные для i-vts
 ...
 int    ExtLabel            = 0;
 int    ExtCodLabel         = 116;
 int    NumberOfBars        = 1000;     // Количество баров обсчёта (0-все)
 int    NumberOfVTS         = 13;
 int    NumberOfVTS1        = 1;
...
//--------------------------------------
 int    ExtFiboType       = 1;
 string ExtFiboTypeFree  = "0,0.382,0.618,0.764,1,1.236,1.618"; // пользовательские уровни фибо 
...
mTypealAP - выбор типа внешних вил 
...
malHandAP - задание прорисовки инструментов вил, выведенных вручную, при задании вывода меток при пересечении с данными вилами
...
// mSelectVariantsPRZ ExtMasterPitchfork       nameObjAPMaster="Master_"+nameObj;
   ...
   k=0;
   if (mTypealAP==0)       // динамические или статические вилы текущего комплекта (противоположные базовым)
     {
   ...
   else if (mTypealAP==1)  // сохраненные вилы из текущего комплекта
     {
      for (i=0; i<j; i++)
        {
         name=ObjectName(i);
   ...
   else if (mTypealAP==2)  // любые вилы из текущего комплекта
     {
      if (mTypeBasiclAP==0)
        {
         for (i=0; i<j; i++)
   ...
   else if (mTypealAP==3)  // статические вилы из других комплектов ZUP с текущего графика
     {
      for (i=0; i<j; i++)
        {
         name=ObjectName(i);
   ...
   else if (mTypealAP==4)  // динамические вилы из других комплектов ZUP с текущего графика
     {
      for (i=0; i<j; i++)
        {
         name=ObjectName(i);
   ...
   else if (mTypealAP==5)  // любые вилы из других комплектов ZUP с текущего графика
     {
      for (i=0; i<j; i++)
        {
         name=ObjectName(i);
   ...
   else if (mTypealAP==6)  // вилы с текущего графика, выведенные вручную, не с помощью ZUP
     {
      for (i=0; i<j; i++)
        {
         name=ObjectName(i);
   ...
   else if (mTypealAP==7)  // любые внешние вилы
     {
      if (mTypeBasiclAP==0)
        {
         for (i=0; i<j; i++)
