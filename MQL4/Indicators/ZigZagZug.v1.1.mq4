...
extern string strPrimarySettings="--- Primary Indicator Settings ---";
//---- Define ZigZag Parameters
extern int    intDepth            = 12;        // Depth Search of ZigZag Extreems
extern int    intBackStep         = 3;         // BackStep for ZigZag Extreems
extern double dblDeviation=5.0;                // Price Deviation of ZigZag Extreems
extern bool   boolShowBreakOut = false;        // Show where Breakouts occured
extern bool   boolAlertOnBreakOut = false;     // Alert User of a Breakout
extern string strBreakoutMsgUpper = "(High) Breakout has occured!";
extern string strBreakoutMsgLower = "(Low) Breakout has occured!";
...
bool boolDownloadHistory=false;    // Download History Data
int intLevel            = 3         // For Depth Recounting
...
int start()
  {
   ...
   intLimitBars,   intCountedBars  = IndicatorCounted(),
    intCounter, intSearch = _SEARCH_BOTH_,intLowPosition,intHighPosition,intShift,intBack,intIndex;
   double dblResult,dblValue,dblCurrentLow,dblCurrentHigh,dblPreviousLow,dblPreviousHigh;
   ...
   if(intCountedBars>0)
     {
      ...
      for(intIndex=intLimitBars-1; intIndex>=0; intIndex--)
        {
         if (intIndex<ArraySize(dblArrayZigZag)) dblArrayZigZag[ intIndex ]          = NULL;
         if (intIndex<ArraySize(dblArrayZigZagLow)) dblArrayZigZagLow[ intIndex ]       = NULL;
         if (intIndex<ArraySize(dblArrayZigZagHigh)) dblArrayZigZagHigh[ intIndex ]      = NULL;
         if (intIndex<ArraySize(dblArrayBreakOutLow)) dblArrayBreakOutLow[ intIndex ]     = NULL;
         if (intIndex<ArraySize(dblArrayBreakOutHigh)) dblArrayBreakOutHigh[ intIndex ]    = NULL;
        }
     }
