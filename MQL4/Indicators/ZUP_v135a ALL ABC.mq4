...
#include <Harmonic Definition.mqh>
...
extern int    maxDepth                = 25;          // исправлены cypher  и А cypher, ABCD - ДОБАВЛЕН КОД УЛУЧШЕНИЯ ОТ grandaevus ( http://www.forex-tsd.com )   
color  ExtColorPatterns        = DarkOrange; // Но добавленные паттерны -  без режима 7 
int    AlgorithmSearchPatterns = 0;
int    SelectPattern           = 0;
...
int    CustomPattern           = 2; // 0 - не выводится пользовательский паттерн 
                                           // 1 - выводится вместе с основной группой паттернов
                                           // 2 - выводится только custom паттерны
double minXB                   = 0.128; // 0.382;  //0.5; это "ABC" поручик 28.04.13 
double maxXB                   = 0.886; // 0.942;  //0.618;
double minAC                   = 0.382; // 0.447;  //0.382;
double maxAC                   = 0.886; // 0.942;  //0.618;
double minBD                   = 1.232; // 1.144;  //1.128;
double maxBD                   = 2.618; // 2.128;  //1.272;
double minXD                   = 0.5; // 0.5;    //0.618;
double maxXD                   = 1.618; // 0.942;  //0.886;
...
int    mTypeBasiclAP           = 0;
int    mTypeExternalAP         = 0;
int    mExternalHandAP         = 0;
...
bool   bigText           = true;
int    bigTextSize       = 16;
color  bigTextColor      = Blue;
color  bigTextColorBearish = Blue;
int    bigTextX          = 50;
int    bigTextY          = 10;
bool   ExtVisible        = true;
int    ExtComplekt       = 13335;
//===================================
...
{1.272, 1.618, 1.618, 1.618, 1.128,
 ...
 1.272, 1.618, 1.618, 1.128,
 1.618, 2.0,   1.618, 1.128,
      ...
      if (StringLen(vNamePattern)>0 &&  (aXABCD[D] < bartoD+2 || patternInfluence==2) || (iu>=0 && PotencialsLevels_retXD_))
        {
         ...
         for(int v=0;v<Bars;v++){
               ...
               else if(current!=0 && zz[v]>0){
                  start = zz[v];
                  }
               else if(current!=0 && start != 0){
                  GlobalVariableSet(StringConcatenate("CurrentZZ_135",Symbol(),Period()),NormalizeDouble(current,Digits));
                  GlobalVariableSet(StringConcatenate("CurrentZZStart_135",Symbol(),Period()),NormalizeDouble(start,Digits));
                  break;
                  }
              else{
                  continue;
         ...
         xaLeg = zz[aXABCD[X]] - zz[aXABCD[A]]; MathAbs(xaLeg); NormalizeDouble(xaLeg,Digits);
         abLeg = zz[aXABCD[A]] - zz[aXABCD[B]]; MathAbs(abLeg); NormalizeDouble(abLeg,Digits);
         bcLeg = zz[aXABCD[B]] - zz[aXABCD[C]]; MathAbs(bcLeg); NormalizeDouble(bcLeg,Digits);
         cdLeg = zz[aXABCD[C]] - zz[aXABCD[D]]; MathAbs(cdLeg); NormalizeDouble(cdLeg,Digits);
         ...
         if (StringLen(vBullBear)>0)
           {
            if (visibleABCDrayZZ && StringFind(vNamePattern,"AB=CD",0)>-1)
            ...
            else
              {
               ...
               ObjectSet(nameObj1,OBJPROP_COLOR,PowderBlue);
               ...
               ObjectSet(nameObj2,OBJPROP_COLOR,LightGreen);
              }
           }
         ...
         if (RangeForPointD>0 && LevelForDmin > 0) // Вывод прямоугольника для зоны точки D
           {
            ...
            string dmintime,dmtest1,dmtest2,dmaxtime;
            int dminTime, dmaxTime;
            dmintime = TimeToStr(TimeForDmin); dmtest1 = StringConcatenate("",StringSubstr(dmintime,0,4),StringSubstr(dmintime,5,2),StringSubstr(dmintime,8,2),StringSubstr(dmintime,11,2),".",StringSubstr(dmintime,14,2));
            dminTime = StrToDouble(dmtest1);
            dmaxtime = TimeToStr(TimeForDmax); dmtest2 = StringConcatenate("",StringSubstr(dmaxtime,0,4),StringSubstr(dmaxtime,5,2),StringSubstr(dmaxtime,8,2), StringSubstr(dmaxtime,11,2),".",StringSubstr(dmaxtime,14,2));
            dmaxTime = StrToDouble(dmtest2); 
            GlobalVariableSet(StringConcatenate("PRZ_BEGIN_135",Symbol(),Period()),dminTime);
            GlobalVariableSet(StringConcatenate("PRZ_END_135",Symbol(),Period()),dmaxTime);
            GlobalVariableSet(StringConcatenate("PRZ_LOW_135",Symbol(),Period()),NormalizeDouble(LevelForDmin,MarketInfo(Symbol(),MODE_DIGITS))); // my code
            GlobalVariableSet(StringConcatenate("PRZ_HIGH_135",Symbol(),Period()),NormalizeDouble(LevelForDmax,MarketInfo(Symbol(),MODE_DIGITS))); // my code
